...
public class CObjPlayer extends CObjBase
{
  ...
  */
  ...
  public CObjPlayer(CLevel _level, CXSprite _sprite, int _x, int _y, int _stance)
  {
    ...
    mActions = mLevel.mPlayerActions;
  ...
  public void ComputeAction(int _key)
  {
    ...
    */
    ...
    if ((_key & 512) != 0)
    {
      ...
      if (mCanDescendLadderDown && (Does(A_STOP_LEFT) || Does(A_STOP_RIGHT) || Does(A_IDLE_LEFT) || Does(A_IDLE_RIGHT) || Does(A_CROUCH_LEFT) || Does(A_CROUCH_RIGHT)))
      {
    ...
    if ((_key & 32) != 0)
    {
      ...
      {
        int lFramesDiff = mAnimFrame -
            mActions.GetAnimFrameStart(mCurrentAction);
        SetA2(A_RUN_RIGHT);
...
*/
  ...
  {
    if (mBB.mBottom >= (mLevel.mMapHeight << 4) - 16)
    {
  ...
  public final void EndCollision()
  {
    ...
    {
      CPoint lPoint = (CPoint)mLevel.mGrabPoints.elementAt(i);
    ...
    {
      CPoint lPoint = (CPoint)mLevel.mCloseGrabPoints.elementAt(i);
  ...
  public final void ManageObjCollision(CObjBase _obj)
  {
    ...
    */
    ...
    if (_obj instanceof CObjTrap)
    {
      ...
      // add an explosion
      mLevel.AddObject(new CObjAnimation(mLevel, mExplosionSprite, _obj.mRealX + _obj.GetWidth() / 2, _obj.mRealY + _obj.GetHeight() / 2, true, 5, Graphics.HCENTER | Graphics.VCENTER, 0));
    }
    ...
    if (_obj instanceof CObjAISlasher)
    {
      ...
      // check if the player is hiding or climbing ladders
      if (Does(A_HIDE_LEFT) || Does(A_HIDE_RIGHT) || Does(A_HIDE_WITH_WEAPON_LEFT) || Does(A_HIDE_WITH_WEAPON_RIGHT))
      {
        return;
      }
  ...
  private final void Flame(int _flameDirection)
  {
    ...
    mLevel.AddObject(new CObjAnimation(mLevel, mFlameSprite, lFlameX, lHotY, true, 5, lFlameAlign, lFlameEffect));
  }
  ...
  {
    mLevel.InsertObject(new CObjAnimation(mLevel, mBlinkSprite, GetMidX(), GetMidY() - 4, true, 8, Graphics.HCENTER | Graphics.VCENTER, 0));
  }
