...
public class CObjBullet extends CObjBase
{
  ...
  {
    if (mRealX < 0 || mRealX > (mLevel.mMapWidth << 4) || mRealY < 0 || mRealY > (mLevel.mMapHeight << 4))
    {
    ...
    */
    ...
    int lColData = mLevel.GetCollisionMap(GetMidX() >> 4, mRealY >> 4);

    if ((lColData & CLevel.LEFT) != 0 || (lColData & CLevel.RIGHT) != 0)
    {
