...
public class CXSprite
{
 ...
 private final boolean Init(String _spriteFile)
 {
   try
   {
     InputStream lIs = getClass().getResourceAsStream(CUtility.getInstance().getFileName(_spriteFile));

     lIs.skip(CUtility.getInstance().GetLocation(_spriteFile));
     ...
     mPixels = new byte[CUtility.getInstance().GetSize(_spriteFile) - (1 + 1 + mColorCount * 3 + mCellsCount * 8)];
     ...
     for(int i = 0; i < mCellsCount; i++)
     {
       ...
       int lWidth = CUtility.BuildShort(lIs.read(), lIs.read());
       int lHeight = CUtility.BuildShort(lIs.read(), lIs.read());
