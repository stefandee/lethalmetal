...
public class CObjAIShooter extends CObjBase
{
  ...
  public CObjAIShooter(CLevel _level, CXSprite _sprite, int _x, int _y, int _stance, int _health, int _damage, int _recoveryTime, int _joyTime, int _aggresivity, int _patrolDistance, int _bulletsDrop)
  {
    ...
    mPlayer = mLevel.mPlayer;

    mActions = mLevel.mAIShooterActions;
    ...
    // CODE SIZE REDUCTION
    mMaxExtentX = Math.min(mLevel.mMapWidth << 4, mStartX + (mPatrolDistance << 4));
  ...
  public void Update()
  {
  	//System.out.println("CObjAIShooter.Update");
  ...
  public void ManageObjCollision(CObjBase _obj)
  {
    ...
    //{
      ...
      if (mHP <= 0)
      {
        ...
        if (mBulletsDrop > 0)
        {
          mLevel.InsertObject(new CObjPowerUp(mLevel, mLevel.mSpriteManager.AddSprite("BOXB"), GetMidX(), mBB.mBottom - 10, CObjPowerUp.POWER_AMMO, mBulletsDrop));
        }
      }
      else
      {
  ...
  private final void Blink()
  {
    ...
    mLevel.InsertObject(new CObjAnimation(mLevel, lSprite, GetMidX() - 4, GetMidY() - 2, true, 8, Graphics.HCENTER | Graphics.VCENTER, 0));
    mLevel.InsertObject(new CObjAnimation(mLevel, lSprite, GetMidX() + 4, GetMidY() - 2, true, 8, Graphics.HCENTER | Graphics.VCENTER, 0));
    mLevel.InsertObject(new CObjAnimation(mLevel, lSprite, GetMidX(), GetMidY() + 4, true, 8, Graphics.HCENTER | Graphics.VCENTER, 0));
  }
}