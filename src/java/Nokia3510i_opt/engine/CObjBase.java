...
public class CObjBase
{
  ...
  // Identification
  //
  public final static int OBJ_TRIGGER_END   = 1;
  public final static int OBJ_TRIGGER_TEXT  = 2;
  public final static int OBJ_TRIGGER_SAVE  = 3;
  ...
  public void Paint(Graphics g, int _x, int _y)
  {
    ...
    if (_x + 96 < mBB.mLeft || 65 + _y < mBB.mTop)
    {
      return;
    }
  ...
  public final void TileCollision()
  {
    ...
    lMaxY = Math.min((mLevel.mMapHeight << 4) - 16, lMaxY);
    ...
    for (int x = lMinX; x <= lMaxX; x++)
    {
      ...
      {
          int lColData = mLevel.GetCollisionMap(x, y);
          int y16 = (y << 4);
          ...
          if ((x16 < lMidX) && (lMidX < x16 + 16))
          {
            ...
            if ((lColData & CLevel.HEAD_LADDER) != 0)
            {
          ...
          else if (mPrevBB.mTop >= y16 + 16)
          {
            ...
            if ( (lColData & CLevel.LEFT) != 0 && mBB.mLeft < x16)//(mBB.mLeft - mPrevBB.mLeft >= 0))
            {
            ...
            if ( (lColData & CLevel.RIGHT) != 0 && mBB.mLeft > x16)//(mPrevBB.mLeft - mBB.mLeft >= 0))
            {
              if ( (mActions.GetAlign(mCurrentAction) & Graphics.RIGHT) == 0)
              {
